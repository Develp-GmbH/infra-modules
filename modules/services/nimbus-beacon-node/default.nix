{
  ...
  inherit (lib) mdDoc mkEnableOption mkOption types;
...
in {
  options.services.nimbus-beacon-node = {
    enable = mkEnableOption "Nimbus Eth2 Beacon Node service.";

    package = mkOption {
      type = types.package;
      default = pkgs.nimbus_beacon_node;
      defaultText = lib.literalExpression "pkgs.nimbus-eth2";
